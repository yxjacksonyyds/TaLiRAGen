...
class VinaDockingTask(BaseDockingTask):
    ...
    @classmethod
    def from_generated_molecule(cls, ligand_rdmol, protein_path, **kwargs):
        # load original pdb
...
if __name__ == '__main__':
    lig_pdbqt = '/root/IRDiff/utils/evaluation/data/AF-P08709-F1-model_v4.pdb'
    # mol_file = 'data/1a4k_ligand.sdf'
    # a = PrepLig(mol_file, 'sdf')
    mol_file = 'CN1CC(C(=O)N(C)Cc2c(F)c(NC(=O)OC(CN)C(=O)[C@@H]3Nc4ccc5c(N)nccc5c4)cc(F)c2O[C@H]2CCOC2)CC(=O)N(C)C1=O'
